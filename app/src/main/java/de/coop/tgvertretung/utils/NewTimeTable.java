...
public class NewTimeTable implements Serializable {
    ...
    HashMap<Week, ArrayList<ArrayList<TimeTableDayEntry>>> weeks = new HashMap<>();
    ...
    public static JsonObject getJsonEntry(TimeTableDayEntry entry, TimeTableDayEntry entryB){
        ...
        if(notEmpty(entry.subject)){
            ...
            if(notEmpty(entry.room))
                jsonObject.addProperty("r", entry.room);
            if(notEmpty(entry.teacher))
                jsonObject.addProperty("t", entry.teacher);
        }
        if(entryB != null){
